...
public class DefaultSession implements Session {
    ...
    public <T> T get(Get get, Class<T> type) throws IOException,
            IllegalAccessException, InstantiationException, InvocationTargetException, ExecutionException {
     ...
     */
    @Override
    public <T> List<T> getList(List<Get> gets, Class<T> type) throws IOException, IllegalAccessException, InvocationTargetException, InstantiationException, ExecutionException {
        final Mapper mapper = this.getMapper(type);
	TableName tableName = TableName.valueOf(mapper.getTableName());
	Table table = getConnection().getTable(tableName);
     ...
     */
    @Override
    public <T> List<T> scan(Scan scan, Class<T> type) throws IOException, IllegalAccessException, InvocationTargetException, InstantiationException, ExecutionException {
        final Mapper mapper = this.getMapper(type);
	TableName tableName = TableName.valueOf(mapper.getTableName());
	Table table = getConnection().getTable(tableName);
     ...
     */
    @Override
    public <E> void put(E e, byte[] rowKey) throws IOException, IllegalAccessException, ExecutionException {
    ...
    @Override
    public <E> void put(Map<byte[], E> map, Class<E> type) throws IOException, IllegalAccessException, ExecutionException {
        final Executor executor = ExecutorFactory
